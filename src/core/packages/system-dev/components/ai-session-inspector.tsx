...
import type { AIAceToolDescriptor, AISessionRuntime, AITurn, AIEntry, AIContextEntry, AILongTermStorageConfig } from '#/schemas/ai';
...
function extractGatewayRequestPrompt(entry: AIEntry): string | undefined {
    const requestBody = entry.request_metrics?.request?.body;
...
function AgentRuntimeTracePanel({ entry }: { entry: AIEntry }) {
    const headers = entry.request_metrics?.response?.headers;
...
function NetworkTracePanel({ entry }: { entry: AIEntry }) {
    ...
    const trace = entry.request_metrics;
...
function LongTermMemorySection({ storage }: {
    storage: AILongTermStorageConfig;
}) {
    const [open, setOpen] = useState(false);
    ...
    return (
        <div className="mb-4">
            ...
            {open && (
                <div className="space-y-2 pl-2 text-[10px]">
                    <div className="border border-fuchsia-700/30 bg-fuchsia-950/10 rounded px-2 py-2">
                        <div className="text-zinc-500 uppercase tracking-wide mb-1">Root</div>
                        <div className="text-fuchsia-300 break-all">{storage.root_dir}</div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div className="border border-zinc-800 rounded px-2 py-2 bg-zinc-900/40">
                            <div className="text-zinc-500 uppercase tracking-wide mb-1">Memories</div>
                            <div className="text-zinc-300 break-all">{storage.memories_dir}</div>
                        </div>
                        <div className="border border-zinc-800 rounded px-2 py-2 bg-zinc-900/40">
                            <div className="text-zinc-500 uppercase tracking-wide mb-1">History</div>
                            <div className="text-zinc-300 break-all">{storage.history_dir}</div>
                        </div>
                        <div className="border border-zinc-800 rounded px-2 py-2 bg-zinc-900/40">
                            <div className="text-zinc-500 uppercase tracking-wide mb-1">Artifacts</div>
                            <div className="text-zinc-300 break-all">{storage.artifacts_dir}</div>
                        </div>
                    </div>
                    <div className="text-zinc-500 italic">
                        Tool outputs and reusable memory are expected to be delegated to LangGraph persistence under these thread-scoped folders.
                    </div>
                </div>
            )}
        </div>
...
function SessionCard({ session }: { session: AISessionRuntime }) {
    ...
    return (
        <div className="border border-zinc-700/50 rounded-lg mb-4 overflow-hidden">
            ...
            >
                <span className="text-zinc-500">{open ? '▾' : '▸'}</span>
                <span className="text-emerald-300 font-bold text-xs">Thread</span>
                ...
                <div className="px-3 py-3 space-y-4 bg-zinc-950/50">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div className="bg-zinc-900 rounded p-2 space-y-1">
                            ...
                            <div><span className="text-zinc-500">thread:</span> <span className="text-zinc-300 font-mono">{session.thread_id}</span></div>
                        </div>
                        <div className="bg-zinc-900 rounded p-2 space-y-1">
                    ...
                    <LongTermMemorySection storage={session.long_term_storage} />
...
export default function AISessionInspector() {
    const [sessions, setSessions] = useState<AISessionRuntime[]>(() => readSessionsFromMemory());

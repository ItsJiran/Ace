...
class EventEngineSingleton {
    ...
    private EventListeners: ListenerMap = new Map<
        EventSlug,
        Map<ListenerUid, ListenerHandler<any, any>>
    >();
    ...
    >(
        ...
        if (!target) {
            await this.emitLocal(slug, event_data);
            return;
        }
        ...
        await this.emitAcrossRuntime(target, slug, event_data);
    }

    private async emitLocal<
        TEventPayload extends Record<string, unknown> = Record<string, unknown>,
        TEventMeta extends Record<string, unknown> = Record<string, unknown>,
    >(slug: EventSlug, event_data: EventData<TEventPayload, TEventMeta>) {
        const listeners = this.EventListeners.get(slug);
        ...
        KernelEngine.writeMemory(this.eventStreamMemoryUid, [
            ...(KernelEngine.readMemory(this.eventStreamMemoryUid) as Array<
        ...
        listeners.forEach((listener) => {
            try {
                Promise.resolve(listener(event_data)).catch((err) =>
                    console.error(`[EventBus] Async handler crashed on slug ${slug}:`, err),
                );
            } catch (error) {
    ...
    private async emitAcrossRuntime<
        TEventPayload extends Record<string, unknown> = Record<string, unknown>,
        TEventMeta extends Record<string, unknown> = Record<string, unknown>,
    >(target: RuntimeTarget, slug: EventSlug, event_data: EventData<TEventPayload, TEventMeta>) {
        const currentRuntime = this.resolveCurrentRuntime();
    ...
    private async relayIncomingRuntimeEvent(event: CrossRuntimeEventMessage) {
        ...
        await this.emitLocal(event.slug, eventData);
    }

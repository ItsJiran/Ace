        ...
        <div className="flex gap-2">
            ...
            <div className="flex flex-col items-center shrink-0">
                ...
                <div className={[
                    ...
                    fgColor?.replace('text-', 'bg-') + '/15',
                ].join(' ')}>
                    <span className={fgColor}>{icon}</span>
                </div>
                {/* Vertical chain line (skip for last block) */}
                {!isLast && (
                    <div className={['w-px flex-1 min-h-[6px]', lineColor || 'bg-zinc-700/40'].join(' ')} />
                )}
            </div>

            {/* Content panel */}
            <div className={['flex flex-col gap-0.5 min-w-0 flex-1', !isLast ? 'pb-2' : ''].join(' ')}>
                ...
                <div className={['border-l pl-2 py-0.5 rounded-l-sm', borderColor || 'border-zinc-700/40'].join(' ')}>

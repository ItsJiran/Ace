 ...
 *
 * Identity check:
 *  The /health endpoint must return `{ ok: true, gateway_name: "ace-deepagent-gateway-server" }`.
 *  Any other HTTP server on that port is silently rejected.
 */

import { KernelEngine } from '../kernel-engine';
import type { AIGatewaySidecarHealthResult, AIGatewayRadarScanResult } from '../../schemas/ai-gateway';

const DEFAULT_BASE_URL = 'http://127.0.0.1:8888';
const GATEWAY_SERVER_NAME = 'ace-deepagent-gateway-server';
    ...
    async radarScan(startPort = 8888, endPort = 8930): Promise<AIGatewayRadarScanResult> {
        ...
        const result: AIGatewayRadarScanResult = {
            ...
            error_message:
                foundPorts.length > 0
                    ? undefined
                    : 'No verified deepagent-gateway-server found in scanned range.',
        };

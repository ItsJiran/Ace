...
2. A backend session is resolved or created. That session owns the durable
    ...
    ACE tools, and the current mirrored context for the active request.
...
5. The runtime binds session-aware gateway tools to the current backend session.
    Those tool callables can mutate plan state, discovered tools, context state,
    and retained memory while the model is running.
...
This keeps cognition, planning, session state, and tool discovery owned by the
...
class DeepAgentRuntime:
    ...
    @staticmethod
    def _prepare_session_for_request(session_state: GatewaySessionState, prompt_kind: str) -> None:
        session_state.active_agent = "coordinator"
    ...
    def _build_agent_invocation(
        ...
        active_profile = get_active_profile(self._coordinator_profile)
    ...
    def _build_debug_prompt_event(
        ...
        return {
            ...
            "payload": {
                ...
                "gateway_agent_active_agent": session_state.active_agent,
                "provider": provider,
                "model": model,
    ...
    @staticmethod
    def _offset_event_indexes(events: list[dict[str, object]], offset: int) -> list[dict[str, object]]:
        if offset <= 0:
            return events
        ...
        for event in events:
            ...
            if isinstance(event_index, int):
                adjusted_events.append({**event, "event_index": event_index + offset})
                continue
    ...
    def build_stream_headers(
        ...
        prompt_kind: str = _USER_PROMPT_KIND,
        context_records: object | None = None,
    ) -> dict[str, str]:
        """Build the initial snapshot headers returned with the HTTP response."""

        session_state = get_or_create_session_state(self._sessions, provider, model, session_uid, ace_tools, context_records)
        self._prepare_session_for_request(session_state, prompt_kind)
    ...
    async def stream_response(
        ...
        try:
            ...
            snapshot_events = self._offset_event_indexes(
            ...
            async for chunk in self._stream_agent_events(
                ...
                prompt,
                session_state,
                invocation_config.profile_name,
                agent,
                stream_state,
            ):
                yield chunk
